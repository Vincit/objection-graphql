...
class SchemaBuilder {
  ...
  extendWithAuthMiddleware(authMiddleware) {
    if (!(authMiddleware instanceof Function)) {
      throw new TypeError('auth middleware should be a function');
    }
  ...
  _preAuthResolver(modelData, extraQuery) {
    const { modelClass } = modelData;
    if (this.authMiddleware) {
      return this.authMiddleware(this._resolverForModel(modelData, extraQuery), modelData, extraQuery);
    }
  ...
  _rootSingleField(modelData) {
    return {
      ...
      args: modelData.args,
      resolve: this._preAuthResolver(modelData, (query) => {
        query.first();
      }),
    };
  }
  ...
  _rootListField(modelData) {
    return {
      ...
      args: modelData.args,
      resolve: this._preAuthResolver(modelData),
    };
  }
