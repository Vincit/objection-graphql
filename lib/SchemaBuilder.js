...
class SchemaBuilder {
  ...
  model(modelClass, opt) {
    ...
    this.modelCache[modelClass.tableName] = {
      modelClass,
      fields: null,
      args: null,
      opt,
  ...
  build() {
    _.forOwn(this.modelCache, (modelData) => {
  ...
  _relationFields(modelData) {
    ...
    _.forOwn(modelData.modelClass.getRelations(), (relation) => {
      const relationModel = this.modelCache[relation.relatedModelClass.tableName];
  ...
  _filterForArgs(astNode, modelClass, variables) {
    ...
    const modelData = this.modelCache[modelClass.tableName];
