...
class SchemaBuilder {
  ...
  extendWithUpload(upload) {
    if (!upload) {
      this.upload = new GraphQLScalarType({
        name: 'Upload',
        description:
    ...
    } else if (!(upload instanceof GraphQLScalarType)) {
      throw new TypeError('upload should be of type GraphQLScalarType');
    } else {
      this.upload = upload;
    }
  ...
  build() {
    ...
    if (this.upload) {
      schemaSetup.upload = this.upload;
    }
