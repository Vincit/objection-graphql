...
class SchemaBuilder {
  ...
  _resolverForMutation(modelData, mode) {
    return (ctx, ignore1, ignore2, data) => {
      ...
      const selectFilter = this._filterForMutations(ast, modelClass, data);
      const builder = modelClass.query(ctx.knex);
  ...
  _filterForMutationArgs(astNode, modelClass, variables, mode) {
    const args = astNode.arguments;
    ...
    return (builder) => {
      switch (recordMode) {
      case 'create': 
        builder.create(argUpdates);
        break;
      case 'update': 
        builder.update(argUpdates);
        break;
      case 'patch': 
        builder.patch(argUpdates);
        break
      case 'delete': 
        builder.delete(argUpdates);
        break;
      default:
        throw new Error(`objection-graphql cannot handle mutation mode ${mode}`);
      }
        // Apply query Args for the filter
